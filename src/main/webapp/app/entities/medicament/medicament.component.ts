...
export class MedicamentComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.medicamentService
            ...
            .subscribe(
                (res: HttpResponse<IMedicament[]>) => this.paginateMedicaments(res.body, res.headers),
    ...
    registerChangeInMedicaments() {
        this.eventSubscriber = this.eventManager.subscribe('medicamentListModification', response => this.reset());
    }
    ...
    protected paginateMedicaments(data: IMedicament[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.medicaments.push(data[i]);
        }
    }
