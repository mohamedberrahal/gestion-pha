...
export class MaladieComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.maladieService
            ...
            .subscribe(
                (res: HttpResponse<IMaladie[]>) => this.paginateMaladies(res.body, res.headers),
    ...
    registerChangeInMaladies() {
        this.eventSubscriber = this.eventManager.subscribe('maladieListModification', response => this.reset());
    }
    ...
    protected paginateMaladies(data: IMaladie[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.maladies.push(data[i]);
        }
    }
